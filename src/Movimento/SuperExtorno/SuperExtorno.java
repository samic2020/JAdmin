package Movimento.SuperExtorno;
...
public class SuperExtorno   implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        btnExtornar.setOnAction(event -> {
            String cSql = "SELECT * FROM caixa WHERE aut = %s;";
            cSql = String.format(cSql, txbAutent.getText());
            ...
            if (existAut) {
                if (Dates.DtDiff(Dates.DIA, Dates.DateFormata("yyyy/MM/DD", cxData), Dates.DateFormata("yyyy/MM/DD", DbMain.getDateTimeServer())) <= 0) {
                    new Alert(Alert.AlertType.INFORMATION, "Você não pode SUPER EXTRONAR itens do dia!\nUse o EXTORNO para itens do dia.").showAndWait();
                    return;
                }
    ...
    private boolean ExtornaREC(String aut) {
        ...
        try {
            while (rs.next()) {
                String uSql = "UPDATE movimento SET aut_rec = null, dtrecebimento = null, usr_rec = null WHERE id = %s;";
