...
public class RecibosController implements Initializable {
    RecebimentoController controller;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        anchorPane.addEventHandler(paramEvent.GET_PARAM, event -> {
            ...
            if (event.sparam.length > 0) {
                if (event.sparam[0] != null) {
                    ...
                    for (String item : venctos.getItems()) {
                        gRecibo recibo = new gRecibo();
                        //recibo.GeraRecibo(contrato.getText(), venctos.getSelectionModel().getSelectedItem().trim());
                        recibo.GeraRecibo(contrato.getText(), item);
                        String svrRecibo = LerValor.BigDecimalToCurrency(recibo.corpoRecibos[recibo.corpoRecibos.length - 1].getVlr());
                        float vrRecibo = recibo.corpoRecibos[recibo.corpoRecibos.length - 1].getVlr().floatValue();

                        // Verifica se lançamentos tem dinheiro para quitar este recibo
                        String[][] LactoRec = null;
                        for (String[] Lacto : Lancamentos) {
                            int posLanc = FuncoesGlobais.FindinArrays(Lancamentos,5,Lacto[5]);
                            if (Lacto[1].equalsIgnoreCase("") && Lacto[2].equalsIgnoreCase("") && Lacto[3].equalsIgnoreCase("")) {
                                float vrLanc = LerValor.StringToFloat(Lacto[5]);
                                if (vrLanc >= vrRecibo) {
                                    float resta = vrLanc - vrRecibo;
                                    if (resta == 0) {
                                        Lancamentos[posLanc][5] = LerValor.FloatToString(resta);
                                        LactoRec = new String[][] {{"","","","","",LerValor.FloatToString(vrRecibo)}};
                                        Lancamentos = FuncoesGlobais.ArrayDel(Lancamentos,posLanc);
                                    } else {
                                        LactoRec = Lancamentos;
                                    }
                                    break;
                                }
                            }
                        ...
                        if (LactoRec == null) LactoRec = Lancamentos;

                        BigInteger aut = conn.PegarAutenticacao();
                        this.Autenticacao = aut;
                        //new Impressao(aut, Lancamentos).ImprimeReciboPDF(dadm, dadosLocador, dadosLocatario, recibo, false);
                        new Impressao(aut, LactoRec).ImprimeReciboPDF(dadm, dadosLocador, dadosLocatario, recibo, false);

                        //Distribuicao(this.rgprp, rgimv.getText(), contrato.getText(), DbMain.GeraLancamentosArray(Lancamentos), valorvenctos.getText());
                        Distribuicao(this.rgprp, rgimv.getText(), contrato.getText(), item, DbMain.GeraLancamentosArray(LactoRec), svrRecibo);
                    }
        ...
        try {
            ...
            controller = (RecebimentoController) loader.getController();
    ...
    private void Distribuicao(String rgprp, String rgimv, String contrato, String vct, String lancamentos, String valor) {
    ...
    private void AutocompleteContratoNome() {
        ResultSet imv = null;
        String qSQL = "SELECT l_contrato, CASE WHEN l_fisjur THEN l_f_nome ELSE l_j_razao END AS l_nome FROM locatarios WHERE exclusao is null ORDER BY l_contrato;";
        try {
    ...
    private void DadosImovel(boolean isOk) {
        String sql = "SELECT p.p_rgprp, p.p_nome, i.i_rgimv, i.i_end || ', ' || i.i_num || ' ' || i.i_cplto AS i_ender, i.i_bairro, i.i_cidade, i.i_estado, i.i_cep, l.l_nrecatz, l.l_ultrecto FROM locatarios l, imoveis i, proprietarios p WHERE (i.exclusao is null) and l.l_contrato = '%s' and l.l_rgimv = i.i_rgimv and l.l_rgprp = CAST(p.p_rgprp AS text)";
