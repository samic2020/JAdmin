...
//        return Float.valueOf(newNumber);
    ...
    public static String Decimal2String(String s) {
        s = s.replace(".","");
        s = s.replace(",",".");
        return s;
    }

    public static String Number2BigDecimal(String value) {
        String tvalue = null;
        if (("" + value).equalsIgnoreCase("")) return "0";
        tvalue = value.replace(".",""); tvalue = tvalue.replace(",",".");
        return tvalue;
    }

    public static String StringValue2Currency(String value) {
        String tvalue = Number2BigDecimal(value);
        String svalue = null;
        int tmp = tvalue.indexOf(".");
        if (tmp == -1 ) {
            svalue = "00";
        } else {
            svalue = tvalue.substring(tmp + 1) + "00";
            svalue = svalue.substring(0,2);
        }
        String pvalue = null;
        if (tmp == -1) {
            pvalue = FuncoesGlobais.StrZero(tvalue.substring(0), 8);
        } else {
            pvalue = FuncoesGlobais.StrZero(tvalue.substring(0, tmp), 8);
        }
        return FormataCurrency(pvalue + svalue);
    }

    public static String BigDecimalToCurrency(BigDecimal valor) { return new DecimalFormat("#,##0.00").format(valor); }
    public static String BigDecimalToCurrencyR$(BigDecimal valor) { return new DecimalFormat("'R$' #,##0.00").format(valor); }
    public static float BigDecimalToFloat(BigDecimal valor) { return valor.floatValue(); }
    public static double BigDecimalToDouble(BigDecimal valor) { return valor.doubleValue(); }
