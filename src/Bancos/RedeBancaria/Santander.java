...
public class Santander {
    ...
    public Santander(DadosBanco Banco) {
    ...
    public void Processa() {
        ...
        for (int i=0; i<pagadors.length; i++) {
            ...
            pagadors[i].setRc_NNumero(StrZero(String.valueOf(fnnumero).replace(".0",""), this.dadosBanco.getBanco_TamanhoNnumero()));
            String codBar = CodBar(pagadors[i].getRc_Vencimento(), vrBoleta, pagadors[i].getRc_NNumero());
            String linDig = LinhaDigitavel(pagadors[i].getRc_NNumero(), pagadors[i].getRc_Vencimento(), vrBoleta);

            pagadors[i].setRc_codigoBarras(codBar);
            pagadors[i].setRc_linhaDIgitavel(linDig);
            pagadors[i].setRc_NNumero(NossoNumero(pagadors[i].getRc_NNumero()));
        ...
        this.dadosBanco.setBanco_NNumero(String.valueOf(fnnumero + 1));
    }

    public void ProcessaAvulsa() {
        Pagador[] pagadors = this.dadosBanco.getBenef_pagadores();
        String nnumero = this.dadosBanco.getBanco_NNumero();
        float fnnumero = 0f;
        for (int i=0; i<pagadors.length; i++) {
            ...
            pagadors[i].setRc_Dados(new String[][] {{"0","kjdshfkjshadkjf aslkjd falkjsdhf lakjsdh lkfasd flakjsdh flkajshd falks","1","1"}});
    ...
    private String CalcDig11NBarrasSantander(String cadeia) {
        int total= 0; int mult = 2;
        for (int i=1; i<=cadeia.length();i++) {
            total += Integer.valueOf(cadeia.substring(cadeia.length() - i,(cadeia.length() + 1) - i)) * mult;
            mult++;
    ...
    private String NossoNumero(String value) {
        String valor1 = StringManager.Right(StrZero("0", this.dadosBanco.getBanco_TamanhoNnumero() - 1) +
    ...
    private String CodBar(String vencimento, String valor, String nossonumero) {
        ...
        part1 = StrZero(String.valueOf(this.dadosBanco.getBanco()), 3) + this.dadosBanco.getBanco_CodMoeda();
        part2 = FatorVencimento(vencimento) + Valor4Boleta(valor) + "9" + this.dadosBanco.getBanco_Conta() + nossonumero + "0" + this.dadosBanco.getBanco_Carteira();
        dv = CalcDig11N(part1 + part2);
        //dv = CalcDig11NBarrasSantander(part1 + part2);
        return part1 + dv + part2;
    }

    private String LinhaDigitavel(String nossonumero, String vencimento, String valortitulo) {
        String campo1, campo2, campo3, campo4, campo5;

        campo1 = StrZero(String.valueOf(this.dadosBanco.getBanco()), 3)  + this.dadosBanco.getBanco_CodMoeda() + "9" + StringManager.Left(this.dadosBanco.getBanco_Conta(), 4);
        campo1 += CalcDig10(campo1);
        campo1 = campo1.substring(0, 5) + "." + campo1.substring(5,10);

        campo2 = StringManager.Right(this.dadosBanco.getBanco_Conta(),3) + StringManager.Left(nossonumero, 7);
        campo2 += CalcDig10(campo2);
        campo2 = campo2.substring(0,5) + "." + campo2.substring(5, 11);

        campo3 = StringManager.Right(nossonumero, 6) + "0" + this.dadosBanco.getBanco_Carteira();
        campo3 += CalcDig10(campo3);
        campo3 = campo3.substring(0, 5) + "." + campo3.substring(5, 11);


        String dvcb = StrZero(String.valueOf(this.dadosBanco.getBanco()), 3)  + this.dadosBanco.getBanco_CodMoeda() + FatorVencimento(vencimento) +
                Valor4Boleta(valortitulo) + "9" + StringManager.Left(this.dadosBanco.getBanco_Conta(), 7) +
                nossonumero + "0" + this.dadosBanco.getBanco_Carteira();
    ...
    private String RemessaHeader240(String nroLote) {
        String codBaco = StrZero(String.valueOf(this.dadosBanco.getBanco()),3);
        String loteSer = "0000";
        String tipoSer = "0";
        ...
        String nomeBan = "BANCO SANTANDER" + Space(15);
    ...
    private String RemessaSegmentoR01240() {
        String codBco = StrZero(String.valueOf(this.dadosBanco.getBanco()),3);
        String loteRe = "0001";
        String tpRems = "1";
        String tpOper = "R";
        String tpServ = "01";
    ...
    private String RemessaSegmentoP240() {
        ...
        try {
            ...
            for (int i = 0; i <= pagadores.length - 1; i++) {
                ...
                // P
                String codBcC = StrZero(String.valueOf(this.dadosBanco.getBanco()),3);
                String nrReme = "0001";
                String tpRegi = "3";
                ...
                String agCedn = StrZero(this.dadosBanco.getBanco_Agencia(),4); //"3405";  // agencia do cedente
                String digAgc = CalcDig11N(this.dadosBanco.getBanco_Agencia());  //"3"; // digito verificador
                String numCoC = StrZero(this.dadosBanco.getBanco_CodBenef(),9); //"013000516"; // Conta Corrente 013000516
                String digCoC = this.dadosBanco.getBanco_CodBenefDv(); //"0"; // digito verificador
                String contCb = StrZero(this.dadosBanco.getBanco_CodBenef(),9); //"013000516"; // Conta cobranca 7926383
                ...
                String valor = String.valueOf(LerValor.StringToFloat(_valor) * 0.00033f);
    ...
    private String RemessaSegmentoQ240(String _nome, String _cpfcnpj, String _ender, String _bairro,
        ...
        String cdbcoc = StrZero(String.valueOf(this.dadosBanco.getBanco()),3);
        String nrltre = "0001";
        String tiporg = "3";
        ...
        String nmesac = myLetra(new Pad(_nome != null ? _nome.toUpperCase() : "", 40).RPad()); //"(40)"; // nome do sacado
        String endsac = myLetra(new Pad(_ender != null ? _ender : "", 40).RPad().toUpperCase()); //"(40)"; // endereco
        String baisac = myLetra(new Pad(_bairro != null ? _bairro : "", 15).RPad().toUpperCase()); // "(15)"; // bairro
        String cepsac = myLetra(new Pad(_cep != null ? _cep.substring(0, 5) : "", 5).RPad().toUpperCase()); // "(5)";  // cep
        String cepsus = myLetra(new Pad(_cep != null ? _cep.substring(6, 9) : "", 3).RPad().toUpperCase()); // "(3)";  // sufixo cep
        String cidsac = myLetra(new Pad(_cidade != null ? _cidade : "", 15).RPad().toUpperCase()); //"(15)"; // cidade
        String ufsaca = myLetra(new Pad(_estado != null ? _estado : "", 2).RPad().toUpperCase()); //"RJ";   // UF
    ...
    private String RemessaSegmentoR0240(String _vencto) {
        // R
        String cbcodc = StrZero(String.valueOf(this.dadosBanco.getBanco()),3);
        String nrlotr = "0001";
        String tporeg = "3";
    ...
    private String RemessaSegmentoS240(String[][] msg) {
        ...
        for (int z=0;z<msg.length;z++) {
            if (msg[z][0] != null) {
                ...
                String msgimp = "4";
                String msgipr = myLetra(new Pad(msg[z][1],30).LPad().toUpperCase() + "  " + new Pad(msg[z][2],7).CPad() + "  " + new Pad(msg[z][3],15).LPad() + Space(44)); //"(100)"; // mensagem a imprimir
    ...
    private String RemessaTrailler50240() {
        String cdgcom = StrZero(String.valueOf(this.dadosBanco.getBanco()),3);
        String nrores = "0001";
        String tporgt = "5";
    ...
    private String RemessaTrailler90240() {
        String cgdcop = StrZero(String.valueOf(this.dadosBanco.getBanco()),3);
        String nrolte = "9999";
        String tpregi = "9";
