package Relatorios.Saldos.Bloqueados;
...
public class Bloqueados implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        btnPreview.setOnAction(event -> {
            ...
            for (Object[] value : sdBloqueados(iniData, fimData)) {
            ...
            parametros.put("NomeRelatorio", "RELATÓRIO DE SALDO DE BLOQUEADOS");
            parametros.put("Periodo", "Período de " + Dates.DateFormata("dd-MM-yyyy", iniData) + " at� " +
                    Dates.DateFormata("dd-MM-yyyy", fimData));

            String pdfName = new PdfViewer().GeraPDFTemp(lista,"SaldosRazao", parametros);
            new PdfViewer("Preview do Saldo de Bloqueados", pdfName);
        });
    }

    private Object[][] sdBloqueados(Date iniData, Date fimData) {
        Object[][] prop = {};
                        ...
                        while (sgrs.next()) {
                            ...
                            if (pos2 == -1) {
                                ...
                                prop = FuncoesGlobais.ObjectsAdd(prop, new Object[] {
                                        ...
                                        dadosprop[0][3].toString(),
                                        seg,
                                        sgrs.getBoolean("retencao") ? seg : new BigDecimal("0")
                                });
                                pos = prop.length - 1;
                            } else {
                                prop[pos2][2] = ((BigDecimal)prop[pos2][2]).add(seg);
                                prop[pos2][3] = ((BigDecimal)prop[pos2][3]).add(sgrs.getBoolean("retencao") ? seg : new BigDecimal("0"));
                            }
                        }
                    } catch (SQLException e) {}
