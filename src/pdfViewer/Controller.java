...
public class Controller {
    ...
    public void initialize() {
        ...
        btnPrint.setDisable(this.FileNamePdf == null);
        btnPrint.setOnAction(event -> {new toPrint(this.FileNamePdf, "LASER",VariaveisGlobais.PrinterMode);});
    }
    ...
    private void updateWindowTitleWhenFileChanges() {
        currentFile.addListener(new ChangeListener<PDFFile>() {
            ...
            public void changed(ObservableValue<? extends PDFFile> observable, PDFFile oldFile, PDFFile newFile) {
                try {
                    String title = VariaveisGlobais.title; //newFile == null ? "PDF Viewer" : newFile.getStringMetadata("Title") ;
    ...
    @FXML private void loadFile() {
        ...
        if (file != null) {
            ...
            VariaveisGlobais.title = this.title;
        }
    }

    public void OpenPdf(String fileOpen) {
        final File file = new File(fileOpen);
        if (file != null) {
            final Task<PDFFile> loadFileTask = new Task<PDFFile>() {
                @Override
                protected PDFFile call() throws Exception {
