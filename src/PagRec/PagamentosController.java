...
public class PagamentosController  implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        btCancelar.setOnAction(event -> {
            if (tipoPagamento == 0) {
                anchorPane.getParent().fireEvent(new paramEvent(new String[] {null}, paramEvent.GET_PARAM));
        ...
        btLancar.setOnAction(event -> {
            ...
            if (valor.subtract(lancado).compareTo(BigDecimal.ZERO) == -1) {
                ...
                if (tipoPagamento > 1) {
                    ...
                    alerta.setContentText("Não existe TROCO para " + (tipoPagamento == 2 ? "Cheque" : "Banco") + "!");
                    alerta.showAndWait();
                    lancado = lancado.subtract(new BigDecimal(LerValor.Number2BigDecimal(valorTextField)));

                    if (tipoPagamento == 2) Cheque();
                    if (tipoPagamento == 3) Banco();
                    return;
                }
            ...
            if (tipoPagamento == 1 && !fimtransacao) {
                // Dinheiro
                Dinheiro();
            } else if (tipoPagamento == 2 && !fimtransacao) {
                // Cheque
                Cheque();
            } else if (tipoPagamento == 3 && !fimtransacao) {
                // Banco
                Banco();
            } else if (fimtransacao) {
                anchorPane.getParent().fireEvent(new paramEvent(new Object[] {"PAGAMENTO",this.cheques}, paramEvent.GET_PARAM));
            }
        });
        ...
        //btLancar.disableProperty().bind(vrresta.textProperty().greaterThan("0,00").and(vrtroco.textProperty().isEqualTo("0,00")));
        btLancar.disableProperty().bind(vrtroco.textProperty().greaterThan("0,00"));
    }
...
*/
    ...
    public void Formas_Disable(boolean disable) {
        ...
        if (!disable) {
            ...
            try {btLancar.setDisable(true);} catch (Exception e) {}
            try {btCancelar.setDisable(false);} catch (Exception e) {}

            Platform.runLater(() -> btDN.requestFocus());
        } else {
            try {btLancar.setDisable(false);} catch (Exception e) {}
            try {btCancelar.setDisable(true);} catch (Exception e) {}
        }
    }
    ...
    private void Cheque() {
        ...
        try {if (!btLancar.isDisabled()) btLancar.setDisable(false);} catch (Exception e) {}
        try {if (!btCancelar.isDisabled()) btCancelar.setDisable(false);} catch (Exception e) {}
    ...
    public BigDecimal GetResta() {
        return new BigDecimal(vrresta.getText().replace(".","").replace(",","."));
    }

    public BigDecimal getValorPago() {
        return this.lancado.setScale(2, BigDecimal.ROUND_HALF_UP);
    }
}
